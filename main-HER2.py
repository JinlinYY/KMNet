...
def main():
    ...
    for train_index, test_index in skf.split(combined_features, label):
        ...
        # 假设 y_test, y_test_pred, test_probs 都是列表或数组
        for idx in range(len(y_test)):
            print(f"Patient {test_patient_indices[idx]} - "
                  f"True label: {y_test[idx]} - "
                  f"Predicted label: {y_test_pred[idx]} - "
                  f"Predicted probability: {test_probs[idx]}")
