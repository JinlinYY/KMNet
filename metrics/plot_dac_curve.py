...
#     fig, axs = plt.subplots(2, 2, figsize=(20, 16))  # 4x2 grid for 7 metrics (no Sensitivity)
#     # plt.subplots_adjust(hspace=0.4, wspace=0.8)
#
#     # Define x and y axis limits
#     axis_limits = [-0.05, 1.05]
#
#     # Define the color scheme for the three classes
#     # class_colors = ['#1f77b4', '#ff7f0e', '#2ca02c']  # You can customize these colors as needed
#     class_colors = ['#9edae5', '#ff7f0e', '#2ca02c', '#d62728']
#     class_labels = ['HER2-low', 'HER2-zero', 'HER2-positive']
#
#     # Plot Precision
#     for i, color in zip(range(n_classes), class_colors):
#         axs[0, 0].plot(thresholds, precisions[i], label=f'{class_labels[i]} Precision', linestyle='-', color=color, lw=3)
#     axs[0, 0].plot(thresholds, avg_precision, label='Average Precision', linestyle='-', color='black', lw=4)
...
#     # Plot Accuracy
#     for i, color in zip(range(n_classes), class_colors):
#         axs[0, 1].plot(thresholds, accuracies[i], label=f'{class_labels[i]} Accuracy', linestyle='-', color=color, lw=3)
#     axs[0, 1].plot(thresholds, avg_accuracy, label='Average Accuracy', linestyle='-', color='black', lw=4)
...
#     # Plot Recall
#     for i, color in zip(range(n_classes), class_colors):
#         axs[1, 0].plot(thresholds, recalls[i], label=f'{class_labels[i]} Recall', linestyle='-', color=color, lw=3)
#     axs[1, 0].plot(thresholds, avg_recall, label='Average Recall', linestyle='-', color='black', lw=4)
#     axs[1, 0].set_title('Recall (Sensitivity)')
...
#     # Plot F1-Score
#     for i, color in zip(range(n_classes), class_colors):
#         axs[1, 1].plot(thresholds, f1_scores[i], label=f'{class_labels[i]} F1-Score', linestyle='-', color=color, lw=3)
#     axs[1, 1].plot(thresholds, avg_f1, label='Average F1-Score', linestyle='-', color='black', lw=4)
...
def plot_dac_curve(y_true, y_probs, dataset_type="Test", save_path=None):
    ...
    # Create subplots for each metric (4x2 grid for 7 metrics now)
    fig, axs = plt.subplots(2, 3, figsize=(22, 12))  # 4x2 grid for 7 metrics (no Sensitivity)
    ...
    class_labels = ['HER2-low', 'HER2-zero', 'HER2-positive']
    # Plot Specificity
    for i, color in zip(range(n_classes), ['#0072C6', '#C60000', '#FFA500']):
        axs[0, 0].plot(thresholds, specificities[i], label=f'{class_labels[i]} Specificity', linestyle='-', color=color)
    axs[0, 0].plot(thresholds, avg_specificity, label='Average Specificity', linestyle='-', color='#9467bd', lw=4)
    ...
    # Plot FNR
    for i, color in zip(range(n_classes), ['#0072C6', '#C60000', '#FFA500']):
        axs[0, 1].plot(thresholds, fnrs[i], label=f'{class_labels[i]}  FNR', linestyle='-', color=color)  # Plot FNR
    axs[0, 1].plot(thresholds, avg_fnr, label='Average FNR', linestyle='-', color='#9467bd', lw=4)  # Plot average FNR
    axs[0, 1].set_title('False Negative Rate (FNR)')
    axs[0, 1].set_xlabel('Threshold')
    ...
    # Plot Precision
    for i, color in zip(range(n_classes), ['#0072C6', '#C60000', '#FFA500']):
        axs[0, 2].plot(thresholds, precisions[i], label=f'{class_labels[i]}  Precision', linestyle='-', color=color)
    axs[0, 2].plot(thresholds, avg_precision, label='Average Precision', linestyle='-', color='#9467bd', lw=4)
    axs[0, 2].set_title('Precision')
    axs[0, 2].set_xlabel('Threshold')
    axs[0, 2].set_ylabel('Precision')
    axs[0, 2].set_xlim(axis_limits)  # Set x-axis limits
    axs[0, 2].set_ylim(axis_limits)  # Set y-axis limits
    axs[0, 2].legend(loc='lower left')
    # axs[2, 0].grid(True)

    # Plot Accuracy
    for i, color in zip(range(n_classes), ['#0072C6', '#C60000', '#FFA500']):
        axs[1, 0].plot(thresholds, accuracies[i], label=f'{class_labels[i]}  Accuracy', linestyle='-', color=color)
    axs[1, 0].plot(thresholds, avg_accuracy, label='Average Accuracy', linestyle='-', color='#9467bd', lw=4)
    ...
    # Plot Recall
    for i, color in zip(range(n_classes), ['#0072C6', '#C60000', '#FFA500']):
        axs[1, 1].plot(thresholds, recalls[i], label=f'{class_labels[i]}  Recall', linestyle='-', color=color)
    axs[1, 1].plot(thresholds, avg_recall, label='Average Recall', linestyle='-', color='#9467bd', lw=4)
    ...
    # Plot F1-Score
    for i, color in zip(range(n_classes), ['#0072C6', '#C60000', '#FFA500']):
        axs[1, 2].plot(thresholds, f1_scores[i], label=f'{class_labels[i]}  F1-Score', linestyle='-', color=color)
    axs[1, 2].plot(thresholds, avg_f1, label='Average F1-Score', linestyle='-', color='#9467bd', lw=4)
    axs[1, 2].set_title('F1-Score')
    axs[1, 2].set_xlabel('Threshold')
    axs[1, 2].set_ylabel('F1-Score')
    axs[1, 2].set_xlim(axis_limits)  # Set x-axis limits
    axs[1, 2].set_ylim(axis_limits)  # Set y-axis limits
...
def plot_dac_curve_2(y_true, y_probs, dataset_type="Test", save_path=None):
    thresholds = np.linspace(0, 1, 101)
